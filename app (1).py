    ...
    with col1:
        ...
        if uploaded_pdfs:
            ...
            if st.button("🤖 Extraer datos con IA", type="primary"):
                ...
                if not api_key:
                    st.error("❌ API Key no encontrada. Configurala en el panel izquierdo.")
                    st.stop()
                st.info(f"🔑 API Key: {api_key[:12]}...{api_key[-4:]} ({len(api_key)} chars)")

                for i, pdf_file in enumerate(uploaded_pdfs):
                    status.markdown(f"**Procesando:** {pdf_file.name}...")
                    try:
                        file_bytes = pdf_file.read()
                        st.info(f"📄 Leyendo {pdf_file.name} ({len(file_bytes)//1024} KB)...")
                        data = extract_from_pdf(
                            api_key,
                            file_bytes,
                            pdf_file.name
                        )
                        st.success(f"✓ JSON recibido: {str(data)[:200]}")
